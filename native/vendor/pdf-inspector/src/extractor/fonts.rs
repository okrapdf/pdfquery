...
fn base14_fallback_widths(doc: &Document, font_dict: &lopdf::Dictionary) -> Option<FontWidthInfo> {
    ...
    for code in 0u16..=255 {
        ...
        if let Some(w) = crate::extractor::base14::base14_char_width(&base_font, ch) {
            widths.insert(code, f32::from(w));
        }
    }
    let space_width = widths.get(&32).copied().unwrap_or(250.0);
...
pub(crate) fn parse_simple_font_widths(
    ...
    if space_width == 0.0 {
        if !widths.is_empty() && (units_scale - 0.001).abs() > 0.0005 {
            ...
            let sum: f32 = widths.values().sum();
            let avg = sum / widths.len() as f32;
            space_width = (avg * 0.45).max(1.0);
        } else {
            space_width = 250.0;
        }
    }
...
pub(crate) fn parse_type0_widths(
    ...
    let space_width = widths
        ...
        .copied()
        .unwrap_or(if default_width > 0.0 {
            default_width / 4.0
        } else {
            250.0
        });
...
mod tests {
    ...
    fn type3_scale_for(matrix_y: f32, bbox_lo: i64, bbox_hi: i64) -> Option<f32> {
        ...
        let font_dict = dictionary! {
            ...
            "Subtype" => "Type3",
            "FontMatrix" => vec![
                Object::Real(matrix_y.into()), Object::Integer(0), Object::Integer(0),
                Object::Real(matrix_y.into()), Object::Integer(0), Object::Integer(0),
            ],
            "FontBBox" => vec![
    ...
    fn make_font_info(widths: &[(u16, u16)], default_width: u16, is_cid: bool) -> FontWidthInfo {
        FontWidthInfo {
            widths: widths
                .iter()
                .map(|(code, width)| (*code, f32::from(*width)))
                .collect(),
            default_width: f32::from(default_width),
            space_width: widths
                .iter()
                .find(|(k, _)| *k == 32)
                .map(|(_, v)| f32::from(*v))
                .unwrap_or_else(|| f32::from(default_width)),
            is_cid,
            units_scale: 0.001,
            wmode: 0,
        }
...
CMapName currentdict /CMap defineresource pop
    ...
    fn parse_cid_w_array_range_and_consecutive() {
        ...
        let w = vec![
            ...
            Object::Integer(500),
            Object::Integer(20),
            Object::Array(vec![Object::Integer(100), Object::Real(653.3203125)]),
        ];
        parse_cid_w_array(&doc, &w, &mut widths);
        assert_eq!(widths.get(&10), Some(&500.0));
        assert_eq!(widths.get(&11), Some(&500.0));
        assert_eq!(widths.get(&12), Some(&500.0));
        assert_eq!(widths.get(&20), Some(&100.0));
        assert_eq!(widths.get(&21), Some(&653.3203125_f32));
    }
    ...
    fn parse_cid_w_array_repeated_full_ranges_stay_bounded() {
        ...
        assert_eq!(widths.get(&65535), Some(&500.0));
    }
}
