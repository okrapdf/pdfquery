...
/// Extract positioned text from an already parsed document.
///
/// This is the same extraction path used by the memory helpers, but lets
/// embedders share one `lopdf::Document` with their own structure traversal.
pub fn extract_text_with_positions_from_document(
    doc: &Document,
    page_filter: Option<&HashSet<u32>>,
) -> Result<Vec<TextItem>, PdfError> {
    extract_text_with_positions_from_document_options(doc, page_filter, false)
}

/// Extract positioned text from an already parsed document, retaining text
/// rendered in PDF mode 3 when `include_invisible` is true.
pub fn extract_text_with_positions_from_document_options(
    doc: &Document,
    page_filter: Option<&HashSet<u32>>,
    include_invisible: bool,
) -> Result<Vec<TextItem>, PdfError> {
    let font_cmaps = FontCMaps::from_doc(doc);
    if let Some(error) = font_cmaps.resource_limit_error() {
        return Err(error);
    }
    let (extraction, _thresholds, _gid_pages) =
        extract_positioned_text_impl(doc, &font_cmaps, page_filter, include_invisible, None)?;
    let (items, _rects, _lines) = extraction;
    Ok(items)
}

#[cfg(test)]
mod pdfquery_document_tests {
    use super::extract_text_with_positions_from_document_options;
    use crate::PdfError;
    use lopdf::{content::Content, content::Operation, dictionary, Document, Object, Stream};

    fn document_with_invisible_text() -> Document {
        ...
        let content = Content {
            operations: vec![
                ...
                Operation::new("Tj", vec![Object::string_literal("Invisible text")]),
    ...
    #[test]
    fn already_parsed_extraction_can_retain_invisible_text() {
        let document = document_with_invisible_text();
        let hidden = extract_text_with_positions_from_document_options(&document, None, false)
            .expect("extract visible text");
        let retained = extract_text_with_positions_from_document_options(&document, None, true)
            .expect("extract all text");

        assert!(hidden.iter().all(|item| item.text != "Invisible text"));
        assert!(retained.iter().any(|item| item.text == "Invisible text"));
    }

    #[test]
    fn positioned_extraction_propagates_decompression_limit() {
        const EXPANDED_BYTES: usize = 50 * 1024 * 1024;
        let mut document = Document::with_version("1.7");
        let pages_id = document.new_object_id();
        let mut bomb = Stream::new(dictionary! {}, vec![b'A'; EXPANDED_BYTES]);
        bomb.compress().expect("compress bomb fixture");
        assert!(bomb.content.len() < 128 * 1024);
        let content_id = document.add_object(bomb);
        let page_id = document.add_object(dictionary! {
            "Type" => "Page",
            "Parent" => pages_id,
        ...
        let error = extract_text_with_positions_from_document_options(&document, None, true)
            .expect_err("decompression bomb must fail extraction");
        assert!(matches!(error, PdfError::ResourceLimit(_)));
    }
}
...
pub(crate) fn text_render_transform(matrix: &[f32; 6], font_size: f32) -> [f32; 6] {
    [
        matrix[0] * font_size,
        matrix[1] * font_size,
        matrix[2] * font_size,
...
pub(crate) fn merge_text_items(items: Vec<TextItem>) -> Vec<TextItem> {
    ...
    for (_, _, group, preserve_stream_order) in &ordered_line_groups {
        ...
        while i < group.len() {
            ...
            while j < group.len() {
                let next = group[j];
                // MCIDs define semantic ownership in a tagged PDF. Merging
                // across this boundary makes downstream structure-tree
                // consumers assign both runs to the first element.
                if next.mcid != first.mcid {
                    break;
                }
...
mod tests {
    ...
    #[test]
    fn merge_items_never_crosses_mcid_boundaries() {
        let mut first = make_merge_item("Alpha", 100.0, 30.0);
        first.mcid = Some(1);
        let mut second = make_merge_item("Beta", 131.0, 24.0);
        second.mcid = Some(2);
        ...
        assert_eq!(merged[0].mcid, Some(1));
        assert_eq!(merged[1].text, "Beta");
        assert_eq!(merged[1].mcid, Some(2));
    }
