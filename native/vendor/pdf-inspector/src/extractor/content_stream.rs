...
pub(crate) fn extract_page_text_items(
    ...
    // Get content
    let content_data = doc.get_page_content(page_id).map_err(PdfError::from)?;
                ...
                if in_text_block && !op.operands.is_empty() {
                    if let Ok(array) = op.operands[0].as_array() {
                        ...
                        let space_threshold = if let Some(font_info) = font_info {
                            let space_em = font_info.space_width * font_info.units_scale;
                    ...
                    || op.operands.is_empty())
                {
                    if let Some(text) = extract_text_from_operand(
                        ...
                        if !text.trim().is_empty() {
                            ...
                            let (x, y) = (combined[4], combined[5]);
                            let width = w_ts_opt
                                .map(|w_ts| w_ts.abs() * combined[0].hypot(combined[1]))
                                .unwrap_or(0.0);
                            let base_font = font_base_names
                                .get(&current_font)
                                .map(|s| s.as_str())
                ...
                if let Some(entry) = marked_content_stack.pop() {
                    if let Some(at) = entry.actual_text {
                        ...
                        if let Some(start_tm) = glyph_tm.or(entry_tm) {
                            ...
                            let width = delta_ts.abs() * combined[0].hypot(combined[1]);
                            if !at.trim().is_empty() {
                                let base_font = font_base_names
                                    .get(&current_font)
                                    .map(|s| s.as_str())
