...
fn analyze_page_content(doc: &Document, page_id: ObjectId) -> Result<PageAnalysis, PdfError> {
    ...
    for content_id in content_streams {
        if let Ok(Object::Stream(stream)) = doc.get_object(content_id) {
            let content = decompressed_content_or_raw(stream)?;
...
pub(crate) fn page_ocr_signals(
    doc: &Document,
    page_id: ObjectId,
) -> Result<(bool, bool), PdfError> {
    let analysis = analyze_page_content(doc, page_id)?;
    ...
    Ok((needs_ocr_for_template_image, analysis.has_vector_text))
}
...
mod tests {
    ...
    fn test_p1_unused_decodable_font_does_not_save_undecodable_page() {
        ...
        let analysis = analyze_page_content(&doc, page_id).unwrap();
        assert!(
