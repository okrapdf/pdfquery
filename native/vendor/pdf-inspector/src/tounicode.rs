...
use crate::is_resource_limit_lopdf_error;
...
impl FontCMaps {
    ...
    fn collect_cmaps_from_fonts(
        ...
        resource_limit_error: &mut Option<String>,
    ) {
        Self::collect_cmaps_from_fonts_inner(fonts, doc, by_obj_num, false, resource_limit_error);
    }
    ...
    fn collect_cmaps_from_fonts_inner(
        ...
        for font_dict in fonts.values() {
            ...
            let data = match stream.decompressed_content() {
                Ok(d) => d,
                Err(error) if is_resource_limit_lopdf_error(&error) => {
                    resource_limit_error.get_or_insert_with(|| error.to_string());
                    continue;
                }
        ...
        for (_name, value) in xobject_dict.iter() {
            ...
            if let Ok(form_resources) = stream.dict.get(b"Resources").and_then(Object::as_dict) {
                ...
                if let Some(font_dict) = font_dict_obj {
                    ...
                    Self::collect_cmaps_from_fonts(&fonts, doc, by_obj_num, resource_limit_error);
                }
    ...
    pub(crate) fn resource_limit_error(&self) -> Option<crate::PdfError> {
        self.resource_limit_error
            .as_ref()
            .map(|error| crate::PdfError::ResourceLimit(error.clone()))
    }
}
