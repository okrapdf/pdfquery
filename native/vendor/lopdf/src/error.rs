...
pub enum Error {
    ...
    /// A decoded stream exceeded the library's bounded decompression budget.
    #[error("decompressed stream exceeds the {limit}-byte limit")]
    DecompressionLimitExceeded { limit: usize },
    /// The decoded content streams for one page exceeded their cumulative budget.
    #[error("page content exceeds the {limit}-byte limit")]
    PageContentLimitExceeded { limit: usize },
