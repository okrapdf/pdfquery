...
/// Maximum cumulative decoded content retained for a single PDF page.
pub const MAX_PAGE_CONTENT_BYTES: usize = 64 * 1024 * 1024;

fn checked_page_content_len(current_len: usize, addition_len: usize, limit: usize) -> Result<usize> {
    current_len
        .checked_add(addition_len)
        .and_then(|length| length.checked_add(1)) // separator newline
        .filter(|&length| length <= limit)
        .ok_or(Error::PageContentLimitExceeded { limit })
}

fn append_page_content(content: &mut Vec<u8>, addition: &[u8], limit: usize) -> Result<()> {
    let new_len = checked_page_content_len(content.len(), addition.len(), limit)?;
    content.reserve(new_len - content.len());
...
#[cfg(test)]
mod page_content_limit_test {
    use crate::Error;

    use super::{append_page_content, checked_page_content_len};

    #[test]
    fn page_content_limit_includes_separator_newlines() {
        const LIMIT: usize = 8;
        let mut content = Vec::new();

        append_page_content(&mut content, b"abc", LIMIT).unwrap();
        append_page_content(&mut content, b"def", LIMIT).unwrap();
        assert_eq!(content, b"abc\ndef\n");

        let result = append_page_content(&mut content, b"", LIMIT);
        assert!(matches!(result, Err(Error::PageContentLimitExceeded { limit: LIMIT })));
        assert_eq!(content, b"abc\ndef\n");
    }

    #[test]
    fn page_content_length_uses_checked_arithmetic() {
        const LIMIT: usize = 64;
        let result = checked_page_content_len(usize::MAX, 1, LIMIT);
        assert!(matches!(result, Err(Error::PageContentLimitExceeded { limit: LIMIT })));
    }
}
        ...
        for object_id in content_streams {
            if let Ok(content_stream) = self.get_object(object_id).and_then(Object::as_stream) {
                match content_stream.decompressed_content() {
                    Ok(data) => append_page_content(&mut content, &data, MAX_PAGE_CONTENT_BYTES)?,
                    Err(error @ Error::DecompressionLimitExceeded { .. }) => return Err(error),
                    Err(_) => append_page_content(&mut content, &content_stream.content, MAX_PAGE_CONTENT_BYTES)?,
                };
            }
        }
        Ok(content)
