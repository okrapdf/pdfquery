...
export function onNode(node: object, type: string, handler: EventHandler<unknown, unknown>): void { const set = setFor(node, type, true)!, stored = handler as StoredHandler; for (const existing of set) if (same(existing, stored)) return; set.add(stored) }
export function oneNode(node: object, type: string, handler: EventHandler<unknown, unknown>): void {
  const once = ((event: PdfEvent<unknown, unknown>) => { offNode(node, type, once); handler(event) }) as StoredHandler; once[originalHandler] = handler; onNode(node, type, once)
}
export function offNode(node: object, type?: string, handler?: EventHandler<unknown, unknown>): void {
...
export function emitNode<T extends object>(node: T, type: string, detail: unknown, hasErrorListener: boolean): unknown[] {
